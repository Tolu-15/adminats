...
export default function MitRegisterPage() {
  ...
  useEffect(() => {
    async function loadBatch() {
      ...
      if (data.programme_type !== 'MIT') { setBatchError('This link is not for an MIT registration.'); return; }
      setBatch(data);
    }
  ...
  async function handleLookup(e) {
    ...
    try {
      const res = await fetch(`/api/mit/lookup?q=${encodeURIComponent(query.trim())}`);
      const json = await res.json();
      if (!res.ok) {
        if (res.status === 403 && json.student) {
          setLookupError(`${json.student.name} (${json.student.student_unique_id}) — Status: ${json.student.status}. You must complete and pass Membership before registering for MIT.`);
        } else {
          setLookupError(json.error || 'Not found.');
        }
      } else {
  ...
  async function handleRegister(e) {
    ...
    setSubmitError('');
    if (!department.trim()) {
      setSubmitError('Department is compulsory for MIT registration. Please enter your department.');
      return;
    }

    setSubmitting(true);
    try {
      const res = await fetch('/api/mit/register', {
        method: 'POST',
      ...
      <div className="reg-card" style={{ maxWidth: 560 }}>
        ...
        <div className="reg-header" style={{ marginBottom: 28 }}>
          ...
          <div style={{
            ...
            padding: '6px 16px', marginBottom: 14, border: '1px solid rgba(212,175,55,0.3)'
          }}>
            <i className="fa-solid fa-book-open" style={{ color: 'var(--gold)', fontSize: '0.85rem' }}></i>
            <span style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--gold)', letterSpacing: 1 }}>MIT</span>
          </div>
          <h1 style={{ fontSize: '1.7rem', color: 'var(--navy)', margin: '0 0 6px' }}>{batch.batch_name}</h1>
          <p className="muted text-sm">MIT Registration — Batch #{batch.batch_code}</p>
        </div>
            ...
            return (
              <div key={i} style={{ flex: 1, textAlign: 'center' }}>
                <div style={{
                  ...
                  background: isDone || isActive ? 'var(--gold)' : 'var(--line)',
        ...
        {step === 2 && foundStudent && (
          <form onSubmit={handleRegister}>
            ...
            <div style={{
              ...
              {foundStudent.photo_url ? (
                <img src={foundStudent.photo_url} alt="Photo"
                  style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid var(--gold)' }} />
              ) : (
                <div style={{
                  width: 72, height: 72, borderRadius: '50%', background: 'var(--gold)', flexShrink: 0,
            ...
            <div style={{
              ...
              borderRadius: 8, padding: '10px 14px', marginBottom: 20, fontSize: '0.85rem', color: 'var(--navy)',
              display: 'flex', alignItems: 'center', gap: 8
            }}>
              <i className="fa-solid fa-circle-info" style={{ color: 'var(--gold)' }}></i>
              <span>Please confirm this is you before proceeding.</span>
            </div>

            {/* Department (Compulsory for MIT) */}
            <div className="field">
              ...
              <p className="muted text-sm" style={{ marginBottom: 8 }}>
                Please specify your church department before registering for MIT.
              </p>
              <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button type="submit" className="btn btn-primary" style={{ flex: 2, padding: '14px' }}
                ...
                {submitting ? (
                  ...
                  <>
                    <i className="fa-solid fa-check"></i> Complete MIT Registration
                  </>
                )}
              </button>
            </div>
          ...
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: '#16a34a', marginBottom: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <i className="fa-solid fa-book-open"></i> Enrolled in MIT · {batch.batch_name}
              </div>
